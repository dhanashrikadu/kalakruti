...
{
    public partial class GrocerProfile : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                using (dbDatabaseDataDataContext db = new dbDatabaseDataDataContext())
                {
                    ...
                    var a = (from s in db.mstGrocerDtls
                             where s.GrocerId == int.Parse(Session["GrocId"].ToString())
                             select s).FirstOrDefault();

                    lblShopmn.Text = a.ShopName;
                    lblSadrress.Text = a.ShopAddr;
                    lblSHOPreg.Text = a.ShopRegNo.ToString();
                    lblSnm.Text = a.ShopOwnName;
                    lblMOb.Text = a.MobNo;
                    lblEmail.Text = a.Email;
                    lblAdhar.Text = a.ShopOwnAdhar;
                    imgprof.ImageUrl = a.image;
                }
            }
