        ...
        protected void BindData()
        {
            ...
            using (dbDatabaseDataDataContext db = new dbDatabaseDataDataContext())
            {
                var get = (from s in db.tbl_DeliveryBoy_Odrs
                           where s.IsDelete == false && s.IsPick == null
                           select new { s.Cust_Address, s.Cust_Mob, s.Paymet_Method, s.Total_Amount, s.Cust_Name, s.Deli_ID, s.ConfirmOdrID }).ToList();
                DlDeliveryBoy.DataSource = get;
                DlDeliveryBoy.DataBind();
            }
        }
                
        protected void Bindgrocer()
        {
          
            using (dbDatabaseDataDataContext db = new dbDatabaseDataDataContext())
            {

                var grocerget = (from z in db.mstGrocerDtls
                                 select new
                                 {
                                     z.ShopName,
                                     z.ShopAddr
                                 }).ToList();
                dlgrocer.DataSource = grocerget;
                dlgrocer.DataBind();
            }
         }
